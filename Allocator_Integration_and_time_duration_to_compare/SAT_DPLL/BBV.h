...
class X
{
    std::byte* ptr;//указатель на ячейку, где хранится нужный бит
    int index;//номер бит   а в ячейке
public:
    X();//конструктор по умолчаню
    X(std::byte* vec, int k);//конструктор с параметрами
...
{
    friend X;
    std::byte* vec;//указатель на массив ячеек
