...
BBV::BBV(int size_v)
{
    if (size_v > 0)
    {
        vec = new std::byte[(size_v - 1) / 8 + 1];//избегаем случай размера кратного восьми
...
BBV::BBV(BBV & V)
{
    if(V.vec != NULL)//проверяем существование вестора V.vec
    {
        ...
        len = V.len;
        vec = new std::byte[size];//захватываем память под новый веторк и переписываем в него значение из переданного вектора
        if (vec != NULL)
        {
...
void BBV::Init(const char * str)
{
    if (str != NULL)//првоериям существует ли строка
    {
        ...
        if (vec != NULL)//если вектор сущестовал, то мы его удаляем
            delete vec;
        vec = new std::byte[size];//захватываем память под новый вектор и переписываем в него значения из строки
        if (vec != NULL)
        {
            int i = 0, j = 8, k = 0;
            std::byte mask = (std::byte)1;
...
BBV BBV::operator=(BBV & V)
{
    if (this != &V)//проверка на самоприсвоение
    {
        ...
        if (vec != NULL)
            delete vec;
        vec = new std::byte[size];//захватываем память под новый вектор и переписывем туда переданный вектор
        if (vec != NULL)
        {
            for (int i = 0; i < size; i++)
                vec[i] = V.vec[i];
...
BBV BBV::operator=(const char * str)
{
    if (str != NULL)//проверяем существует ли строка
    {
        ...
        if (vec != NULL)
            delete vec;
        vec = new std::byte[size];//захватываем память для нового вектора и переписываем в его биты соотв. знач. строки
        if (vec != NULL)
        {
            int i = 0, j = 8, k = 0;
            std::byte mask = (std::byte)1;
...
X::X(std::byte* vec, int k)//передаем в конструктор указатель на нужную ячейку и номер бита в ней
{
...
X X::operator=(int k)
{
    ...
    {
        std::byte mask = (std::byte)1;//подготавливаем маску с правильно размещенным битом со значение 1
        mask <<= index;
...
X::operator int()
{
    if (ptr != NULL)//проверяем сущестование ячейки с нужным битом
    {
        ...
        std::byte mask = (std::byte)1;//формируем маску с выставленной 1 в нужном бите
        mask <<= index;
        if ((int)(*ptr & mask))//если предположение раввенства 0 не верно, запишем в k новое значение 1
            k = 1;
        return k;
    }
    else
...
X X::operator=(X& v)
{
    if (ptr != NULL && v.ptr != NULL)
    {
        std::byte mask = (std::byte)1;//подготавливаем маску с правильно размещенным битом со значение 1
        mask <<= v.index;
        if ((int)(mask & *(v.ptr)))//производим запись в ячейку в нужный бит
            *this = 1;
        else
            *this = 0;
    }
...
ostream & operator << (ostream &r, BBV& V)
{
    if (V.vec != NULL)//првоверяем существование ячеек булева вектора
    {
        ...
        while (i < V.len)//цикл для прохода по всем битам вектора
        {
            ...
            {
                if ((int)(V.vec[k] & mask))//с помощью маски получаем значение бита и выводим его
                    cout << '1';
                else
                    cout << '0';
                mask <<= 1;
